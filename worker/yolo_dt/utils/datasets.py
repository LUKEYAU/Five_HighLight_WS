...
class LoadImages_for_football:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        p = str(Path(path).absolute())  # os-agnostic absolute path
        if '*' in p:
            files = sorted(glob.glob(p, recursive=True))  # glob
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            # Read video
            self.mode = 'video'
            ret_val, while_game_img = self.cap.read()
            ...
            # Read image
            self.count += 1
            while_game_img = cv2.imread(path)  # BGR
            assert while_game_img is not None, 'Image Not Found ' + path
            #print(f'image {self.count}/{self.nf} {path}: ', end='')
        # print(path)
        imgs_for_pred = []
        imgs_for_draw = []
        ...
        for idx in range(4):
            # img = np.array((while_game_img.shape[0] // 2 + 1, while_game_img.shape[1] // 2 + 1, 3))
            if(idx == 0):                                                               #left_top
                cut_img = while_game_img[:while_game_img.shape[0] // 2, :while_game_img.shape[1] // 2, :]
                draw = copy.deepcopy(cut_img)
            elif(idx == 1):                                                             #right_top
                cut_img = while_game_img[:while_game_img.shape[0] // 2, while_game_img.shape[1] // 2:, :]
                draw = copy.deepcopy(cut_img)
            elif(idx == 2):                                                             #left_down
                cut_img = while_game_img[while_game_img.shape[0] // 2:, :while_game_img.shape[1] // 2, :]
                draw = copy.deepcopy(cut_img)
            else:                                                                       #right_down
                cut_img = while_game_img[while_game_img.shape[0] // 2:, while_game_img.shape[1] // 2:, :]
            ...
            # print(cut_img.shape)
                # Padded resize
            img = letterbox(cut_img, self.img_size, stride=self.stride, scaleup = False)[0]

            # Convert
            img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
            img = np.ascontiguousarray(img)
            
            imgs_for_pred.append(img)
            imgs_for_draw.append(draw)
            # print(img.shape)
        

        return path, imgs_for_pred, imgs_for_draw, while_game_img, self.cap
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                # verify labels
                # lb_file = lb_file.replace('mask', 'finetune')
                #print("label checkcheckcheckcheckcheckcheck" + lb_file)
